...
| **Document Version** | 1.2 — Reconciled to the projection/no-AI pivot (SH-01 dashboard→GM report; SH-02 +Console/judge; SH-03 results-screen/tiers→end-of-read/value layer; SH-04 epics→report engine/Console; embedded RACI de-staled) |
...
|  |  |
| --- | --- |
| **Communication Method**  Sprint review briefings at end of each sprint. Pilot debrief meeting (45 minutes, Week 14). **GM Findings Report** self-served at any time during the pilot via **Ctrl+Alt+A + PIN** (auto-lock). | **Notes**  The **GM is the report consumer / buyer**. Final decision authority on Phase 2 continuation. Approves Project Charter. Encourages (but does not mandate) front desk team participation. Primary recipient of findings from the report + debrief. |
...
| **GM report access** | Ctrl+Alt+A + GM PIN (auto-lock) on any front desk PC or via the GuestIQ URL |
...
|  |  |  |
| --- | --- | --- |
| **Interest in Project**  Instrument quality and validity — the **projection instrument surfaces non-obvious, actionable findings** a GM will act on. Publishable findings from the pilot. Phase 2 academic collaboration opportunities. Personal professional development through research output. | **Influence**  **High** | **Engagement Strategy**  Manage Closely — daily collaboration partner, all decisions flow through this role |

|  |  |
| --- | --- |
| **Communication Method**  Daily collaboration with AI Developer in this conversation. Reviews and approves all project artifacts. Facilitates UAT sessions in Sprint 5. Monitors Supabase + the **Researcher Console** daily during Sprint 6 pilot. Receives Sentry error alerts and PostHog weekly digest by email. | **Notes**  Primary human team member covering ALL human roles simultaneously: Project Sponsor liaison, Lead Researcher, UAT Facilitator, Pilot Monitor, and GM Liaison. This role concentration is formally documented as Risk R-11 (High/High). Approves all artifacts within 48 hours. Makes all instrument and research decisions. Single point of contact for front desk team during pilot. **The pilot's judge** (Researcher Value Model v0.2): reviews report-engine integrity and captures the GM's reaction to findings in the Console — a check that never gates the GM's report. Epic Owner for all research-domain Epics (Agent Experience, Research Data Quality, Report & Console). |
...
| **GuestIQ access** | GM report: Ctrl+Alt+A + GM PIN · Researcher Console: researcher PIN (both auto-lock) |
...
|  |  |  |
| --- | --- | --- |
| **Interest in Project**  A genuinely engaging experience that respects their professional expertise and does not waste their limited time. Seeing their input matter — the **end-of-read** reflects their own expertise back as a short, dignified story, making their contribution tangible. Recognition that their hospitality experience is valuable. | **Influence**  **Medium** | **Engagement Strategy**  Keep Informed — engage through experience design, not direct management |

|  |  |
| --- | --- |
| **Communication Method**  Single MS Teams launch message at pilot start (Week 11). Follow-up Teams message on Day 5 if participation is below target. In-person mid-pilot check-in on Day 5 by Lead Researcher. Bug Reporting Guide pinned to Teams channel. The **end-of-read** appears immediately upon completing any read. | **Notes**  Voluntary participation only — never mandated. The **dignified value layer** (badge identity, the five-beat end-of-read, the coverage wall) activates professional self-concept **without gamification**, which would patronise a professional audience. The end-of-read reflection is the primary intrinsic reward. UAT participants (2–3 members) selected from this group for Sprint 5. |
...
|  |  |
| --- | --- |
| **Communication Method**  All communication occurs in this conversation. Artifacts delivered as downloadable Word documents and code files. Responds to all requests within the same session. Bound by approved artifacts — no unilateral scope changes. | **Notes**  Produces all AI deliverables: the project artifacts, the complete React PWA, Supabase schema, Sentry configuration, PostHog event instrumentation, Playwright tests, the **deterministic report engine**, the **RosaeNLG story (local)**, and the **GM report + Researcher Console**. **No third-party AI is built or used.** Does not make decisions — produces options and recommendations for the Lead Researcher to decide. SCRUM MASTER FUNCTION (v1.1): at the start of each sprint session Claude transitions into Scrum Master mode and runs the 7-item checklist before any delivery work — the structural resolution to Risk R-11. EPIC OWNER: owns the technical-domain Epics — Report Engine & GM Report, Researcher Console, Data Infrastructure, PWA Shell & Delivery. *(The former Management Dashboard and Content-Management epics are retired.)* |
...
| **Build v4.2 routing test (9 personas)** | **I** | **A** | **—** | **R** | **—** | **—** | **—** |
| **Build report-engine integrity protocol** | **C** | **A** | **—** | **R** | **—** | **—** | **—** |
| **Design instrument config (v4.2) structure** | **I** | **A** | **—** | **R** | **—** | **—** | **—** |
| **Build the deterministic report engine + RosaeNLG story** | **I** | **A** | **—** | **R** | **—** | **—** | **—** |
...
| **Edit instrument config (v4.2)** | **—** | **A** | **—** | **I** | **—** | **—** | **—** |
| **Review report-engine integrity + capture GM-reaction (Console)** | **—** | **R+A** | **—** | **C** | **—** | **—** | **—** |
