...
export const trackRoutingGateAnswered = (props) => posthog.capture('routing_gate_answered', props);

// ── 5.2 Questionnaire Flow ────────────────────────────────────────────────

/** Event 6 — fires when the first question of an episode renders */
export const trackEpisodeStarted = (props) => posthog.capture('episode_started', props);

/** Event 7 — fires on every answered question (any option, including none) */
export const trackQuestionAnswered = (props) => posthog.capture('question_answered', props);

/** Event 8 — fires IN ADDITION to question_answered when none option selected */
export const trackNoneFlagSelected = (props) => posthog.capture('none_flag_selected', props);

/** Event 9 — fires when the last question of an episode is answered */
export const trackEpisodeCompleted = (props) => posthog.capture('episode_completed', props);

/** Event 10 — fires when curiosity hook screen renders */
export const trackCuriosityHookViewed = (props) => posthog.capture('curiosity_hook_viewed', props);

/** Event 11 — fires when tier upgrade prompt is shown */
export const trackTierUpgradePrompted = (props) => posthog.capture('tier_upgrade_prompted', props);

/** Event 12 — fires when respondent accepts a tier upgrade */
export const trackTierUpgradeAccepted = (props) => posthog.capture('tier_upgrade_accepted', props);

/** Event 13 — fires when respondent declines a tier upgrade */
export const trackTierUpgradeDeclined = (props) => posthog.capture('tier_upgrade_declined', props);

/** Event 14 — fires when Q2 secondary purpose triggers a second Module 5 sub-section */
export const trackPurposeExpert = (props) => posthog.capture('purpose_expert', props);

// ── 5.3 Session Management and Connectivity ───────────────────────────────

/** Event 15 — fires on window.beforeunload when session is incomplete */
export const trackSessionPaused = (props) => posthog.capture('session_paused', props);

/** Event 16 — fires when disambiguation screen renders */
export const trackDisambiguationShown = (props) => posthog.capture('disambiguation_shown', props);
...
export const trackOfflineQueueFlushed = (props) => posthog.capture('offline_queue_flushed', props);

// ── 5.4 Completion and Results ────────────────────────────────────────────

/** Event 21 — fires when all questions for the respondent's tier are answered */
export const trackSessionCompleted = (props) => posthog.capture('session_completed', props);

/** Event 22 — fires when completion results screen renders */
export const trackResultsViewed = (props) => posthog.capture('results_viewed', props);
...
export const trackDashboardOpened = (props) => posthog.capture('dashboard_opened', props);
