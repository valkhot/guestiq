...
| **E6** | Findings Engine | 8 | 46 | ✅ S4 (5 gates, tiering, compute-on-open; report_cache → S5) |
| **E7** | UC-03 GM Findings Report | 4 | 18 | ✅ S5 (GM Report, dossier canvas, PIN-gated) |
| **E8** | UC-04 Researcher Console | 6 | 26 | ✅ COMPLETE — six-lens Console (01 Integrity · 02 Validation · 03 Agent-activity · 04 App-health · 05 GM-activity · 06 Iteration) + toggle + export; 04/05 delivered S6 |
...
| **E12** | Guest Persona Builder (pre-questionnaire socio-demographic anchoring) | — | — | 🅿 **Phase 2** — feature request; needs design + anonymity/bias review (NOT pilot scope) |
| **E13** | Badge pool — scalable identity supply (show-12-of-N + more animal art) | — | — | 🅿 **Phase 2** — needed if agents > 12 or multi-property; art must be hand-authored |
| | **Total (active)** | **51** | **239** | |
...
## Phase 2 — parked feature requests (tracked, not in pilot scope)

### True RosaeNLG narrative (server-side render)
Attempted in Sprint 6 (R1): compiled + rendered + bundled in-browser, but RosaeNLG's runtime needs Node built-ins and wants **server-side rendering**. Shipped a deterministic synthesis story (R2) instead. Phase-2: expose a small render endpoint (or serverless fn) so agents get true RosaeNLG linguistic variation without shipping the Node runtime to the browser. Low priority — R2 already reads well.


### E12 · Guest Persona Builder — pre-questionnaire socio-demographic anchoring

**Source:** feature request `guest_persona_builder_feature_request.docx` (raised Sprint 3 session; proposed epic **UC-08 "Front Desk Agent Answers with Precision"**; proposed story S6-NEW-PERSONA).
**Status:** 🅿 **Phase 2** · LOGGED · **needs design + anonymity/bias review before scoping** · NOT pilot scope · no dev authorised.

**Problem it addresses.** Agents currently answer from a *composite* of every guest they've served → generalised, single-level data. Anchoring each read to a specific guest profile yields a **segmented** dataset ("expectations for 45–54 frequent business travellers"), improving construct validity of the proxy-respondent design and enabling sub-group analysis.

**User story.** *As a front desk professional, I want to quickly build a guest persona from suggested socio-demographic parameters before I answer, so my answers reflect a specific guest type — more precise and more useful for management.*

**Six parameters (each 4–6 options, chips not dropdowns):**
- **Age band** — 18–24 · 25–34 · 35–44 · 45–54 · 55–64 · 65+
- **Travel frequency** — First time · Occasional (1–3×/yr) · Regular (4–10×/yr) · Frequent (10+/yr)
- **Booking channel** — Direct · OTA · Corporate tool · Loyalty · Walk-in
- **Party config** — Solo · Couple · Family (young kids) · Family (teens) · Friends · Corporate group
- **Nationality group** — UK/Ireland · W. Europe · E. Europe · N. America · Middle East · Asia-Pacific · Other
- **Stay purpose** — Work · Leisure · Mixed · Personal necessity *(mirrors Q1 — anchors intent before Q1 is asked)*

**Acceptance criteria (draft, from request):**
1. Appears **after tier-selection, before Q0** (respondent invested first).
2. Card-based selectors; chips, not dropdowns.
3. **All optional** — skip any/all (backward-compatible with Phase-1 sessions).
4. Real-time **persona card** builds as choices are made.
5. Persona card **persists collapsed** in the question-screen header.
6. Stored in Supabase — **Option B (new `persona_sessions` table)** recommended over a JSONB column.
7. Persona dimensions enrich **all PostHog `question_answered` events** for segmented funnels.
8. Welcome screen invites a returning agent to try a **different persona**.
9. ≤ **60s** to complete all 6; ≤ **5s** to skip.

**Technical implications (Phase 2):** new `PersonaBuilderScreen` (between welcome and questionnaire), `usePersona` hook, `PersonaCard` component (full + collapsed), welcome-screen returning-session detection; PostHog enrichment (6 new properties) + new funnel + a Persona-Distribution panel; option copy in content files, not hardcoded.

**⚠ Why Phase 2, not Sprint 6 (my flags):** net-new feature vs. Sprint 6's hardening/pilot scope; it touches the instrument's deliberately *lightweight, no-demographic* model; **demographic anchoring raises anonymity risk** (more identifying structure per read) **and bias risk** (agents projecting stereotypes onto demographic anchors), and changes what findings *mean* — so it needs an explicit anonymity/ethics + bias review before scoping; and it's **best validated after the pilot**, which may show whether more anchoring is needed at all.

### E13 · Badge pool — scalable identity supply
**Source:** raised by Lead Researcher during Sprint 6 QA, after testing exhausted all 12 badges and a new agent could not claim one.

**The concept (Val's):** always present **12 badges on screen**, backed by a **deeper pool** (e.g. +10 pre-authored). When free badges drop below a threshold (~3), **top up** so the picker never runs dry and every new agent gets a full, generous choice of 12 — rather than the last 2 nobody wanted. Claimed badges drop out of the visible grid instead of accumulating as a wall of grey.

**Why it can't be "generated on the fly":** each coin is **hand-drawn SVG line art** (~470 chars of paths); there are exactly **12** (badger, bear, boar, fox, hare, hawk, hedgehog, lynx, owl, ram, tortoise, wolf). Art cannot be produced procedurally at runtime. `Coin.jsx` falls back to **a serif letter** when art is missing — i.e. exactly the placeholder look deliberately replaced in Sprint 1. So the pool must be **pre-authored**, and the "generate on the fly" step becomes **"reveal from a pre-authored pool"** (identical UX).

**Scope when built:**
- Author **N more animals** (line-art SVG matching the existing 12 — honest caveat: hand-drawn animal art is hard; risk of visibly weaker coins dragging the set down; budget real design time).
- `badges` table seeded with the larger pool.
- Picker shows **12 available** (not 12 total); claimed drop out; grid tops up from the pool.
- Optional threshold logic (top up when < 3 free).

**Current mitigation (shipped S6):** **Console → Lens 03 → Badges → Release** (PIN-gated) lets the researcher free a badge when an agent loses access. Plus `GuestIQ-badge-reset.sql` for a pre-pilot clean slate.

**Trigger to build:** front-desk headcount **> 12**, or multi-property. For a single ~10-agent desk, 12 + a clean reset is sufficient — the pool ran dry from *testing*, not from design.

**Related risk (mitigated, not eliminated):** a lost badge (cleared browser / new device) is unreclaimable by the agent → they'd take a second badge → **one human becomes two identities**, corrupting constellation counts and Lens 03. The Console release control is the pilot-grade fix; a durable identity recovery path is a Phase-2 consideration.
